...
# Parte 2: Linguagem Go (Golang) 🐹

**Go**, criada pelo Google, é a linguagem da infraestrutura moderna (Docker e Kubernetes são feitos em Go). Ela foca em **simplicidade** e **concorrência**.

## 🚀 Seu Primeiro Programa em Go

A estrutura é minimalista. Tudo pertence a um "pacote".

```go
// Todo arquivo Go pertence a um pacote. 'main' gera um executável.
package main

// Importamos o pacote de formatação (format)
import "fmt"
...
### 1. Declaração Curta (`:=`)
Go infere o tipo da variável, economizando digitação.

```go
func main() {
    nome := "Carlos" // Go entende que é String
    idade := 28      // Go entende que é Int
...
Funções em Go podem retornar mais de um valor. Isso é muito usado para retornar erros.

```go
func dividir(a, b float64) (float64, error) {
    if b == 0 {
        return 0, fmt.Errorf("divisão por zero impossível")
    }
    return a / b, nil
}
```
...
Go pode rodar milhares de tarefas simultâneas facilmente usando a palavra `go`.

```go
func main() {
    go processarDados() // Roda em segundo plano
    fmt.Println("O programa continua...")
}
```

## Comparativo: Rust vs Go

| Característica | Rust 🦀 | Go 🐹 |
| :--- | :--- | :--- |
| **Foco** | Controle total, Gerenciamento de Memória, Segurança absoluta. | Simplicidade, Produtividade, Servidores de Rede. |
| **Curva de Aprendizado** | Íngreme (difícil de começar). | Suave (fácil de aprender). |
| **Performance** | Nativa (igual C++). | Excelente (perto de C++), mas tem Garbage Collector. |
| **Uso Ideal** | Drivers, Engines de Jogos, Sistemas Críticos. | APIs, Microsserviços, Ferramentas de CLI. |

---
**Próxima Aula**: Vamos explorar o desenvolvimento móvel com **Dart (Flutter) e Kotlin**.
